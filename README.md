...
An unofficial native build of Sonic Lineup for Apple Silicon Macs. The software is that of the Centre for Digital Music, Queen Mary University of London, whose original notes follow below; this fork alters only what current Macs require to build and bundle it.

- **Download.** Each [release](https://github.com/yrammos/sonic-lineup-arm64/releases) carries the zipped application; unzip it and move `Sonic Lineup.app` to Applications. The release notes state the minimum macOS version.
- **First launch.** The app is ad hoc signed, not notarized by Apple, so macOS blocks it at first launch. Allow it under System Settings › Privacy & Security › Open Anyway, or clear the quarantine attribute:
...
- **License.** GPL, as upstream (see `COPYING`); each release is tagged at the commit from which it was built. Upstream repository: [sonic-visualiser/sonic-lineup](https://github.com/sonic-visualiser/sonic-lineup).
